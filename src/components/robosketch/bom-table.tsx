...
export default function BomTable({ bom }: BomTableProps) {
  const totalCost = bom.reduce((acc, item) => acc + item.approximatePriceUSD, 0);
        ...
        <TableBody>
          {bom.map((item, index) => (
            <TableRow key={index}>
              ...
              <TableCell className="text-right">${item.approximatePriceUSD.toFixed(2)}</TableCell>
          ...
          <TableRow className="bg-secondary hover:bg-secondary">
            <TableCell colSpan={2} className="font-bold text-right">Total Approximate Cost</TableCell>
